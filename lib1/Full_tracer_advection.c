...
static int icheck_element_column(struct All_variables *E,
                                 ...
                                 double x, double y, double z,
                                 double rad);
int icheck_bounds(struct All_variables *E,
                         double *test_point,
...
void full_tracer_setup(struct All_variables *E)
{
	...
	if (E->control.melting_model!=0)
		E->trace.number_of_extra_quantities = 7;
...
static void write_trace_instructions(struct All_variables *E)
{
    ...
    if (E->trace.nflavors && E->trace.ic_method==0) {
	  ...
	  /* default mode 0 */
			...
			fprintf(E->trace.fpt,"read chemical from file and then get tracer flavor from it\n");
		}	
#ifdef USE_GGRD
...
/*
    for (i=0; i<E->trace.nflavors-2; i++) {
        if (E->trace.z_interface[i] > E->trace.z_interface[i+1]) {
            fprintf(E->trace.fpt,"Sorry - The %d-th z_interface is bigger than the next one.\n", i);
            fflush(E->trace.fpt);
            parallel_process_termination();
        }
    }
    ...
    if (number_of_tries>10)
        {
            ...
            fprintf(stderr,"Test Point: %f %f %f  \n",test_point[1],test_point[2],test_point[3]);
            fprintf(stderr,"Nodal points: 1: %f %f %f\n",rnode1[1],rnode1[2],rnode1[3]);
            fprintf(stderr,"Nodal points: 2: %f %f %f\n",rnode2[1],rnode2[2],rnode2[3]);
            fprintf(stderr,"Nodal points: 3: %f %f %f\n",rnode3[1],rnode3[2],rnode3[3]);
            fprintf(stderr,"Nodal points: 4: %f %f %f\n",rnode4[1],rnode4[2],rnode4[3]);
            fflush(E->trace.fpt);
            exit(10);
        }
