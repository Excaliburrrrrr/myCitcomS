...
void viscosity_system_input(struct All_variables *E)
{
	...
	input_float("visc_T_ref",&(E->viscosity.T_ref),"1.0",m);//haoyuanli
        /* for viscosity 8 */
    ...
    input_int("use_ne_visc_smooth",&(E->viscosity.SMOOTH),"0",m);
    input_int("use_ne_visc_smooth_test",&(E->viscosity.SMOOTH_test),"0",m);
     ...
     float **evisc,**visc;
{
    ...
    void  visc_from_nodes_to_gint_log();
	...
	if(E->viscosity.RHEOL==250){
		visc_from_TC_by_mineral(E,evisc);
	}
	else{
    	if(E->viscosity.TDEPV)
    ...
    */
    ...
    if(E->viscosity.SMOOTH){ /* go the other way, for
					    smoothing */
	  int nel = E->lmesh.NEL[E->mesh.levmax];
	  for (i=1;i<=E->sphere.caps_per_proc;i++)
	    E->VIextra[i] = (float *) malloc((nel+1)*vpoints[E->mesh.nsd]*sizeof(float));

	  if(E->parallel.me == 0) //lhy new_smoothing_method
	  	fprintf(stderr,"smoothing_on_the_other_way\n"); //lhy debug
      visc_from_nodes_to_gint_log(E,visc,evisc,E->mesh.levmax);
      //visc_from_nodes_to_gint(E,visc,evisc,E->mesh.levmax);
	  for(i=1;i<E->viscosity.SMOOTH;i++){
      	visc_from_gint_to_nodes(E,evisc,E->VIextra,E->mesh.levmax);
      	visc_from_nodes_to_gint_log(E,E->VIextra,evisc,E->mesh.levmax);
	  }
      visc_from_gint_to_nodes(E,evisc,E->VIextra,E->mesh.levmax);
    }
		  ...
		  TT[kk] = E->T[m][E->ien[m][i].node[kk]];
		    ...
		    EEta[m][ (i-1)*vpts + jj ] = tempa*
		      exp( E->viscosity.E[l]/(temp+E->viscosity.T[l])
			   - E->viscosity.E[l]/(E->viscosity.T_ref +E->viscosity.T[l]) );//T_ref is one by default
                }
            }
        break;
     ...
     float **EEta;
{
  ...
  if(Is0){//debug
	  ...
	  fprintf(stderr,"smooth_downwelling =%d",E->lunar.smooth_downwelling);
  }
    ...
    for(i = 1; i <= nel; i++){
	 ...
	 element */
        ...
        for(jj = 1; jj <= vpts; jj++) {
            ...
            if(E->lunar.smooth_downwelling ==0){                                //zwb 20200730
                for(p=0; p<E->composition.ncomp; p++) {
                    vmean += cc_loc[p] * E->viscosity.cdepv_ff[p+1];
                }
            }
            else if(E->sx[1][3][E->ien[m][i].node[kk]]>=E->trace.z_interface[0]){
                for(p=0; p<E->composition.ncomp; p++) {
                    vmean += cc_loc[p] * E->viscosity.cdepv_ff[p+1];
                }
            }
                ...
                for(p=0; p<E->composition.ncomp; p++) {
                    vmean += cc_loc[p] * E->lunar.smooth_cdepv[p+1];
                }
            }
            vmean = exp(vmean);
  ...
  /*if(Is0){//debug
	  fprintf(stderr,"visc_from_C end\n");
  }*/
}
...
double dist_sphere(x1,y1,x2,y2)  //nanzhang2017
double x1,y1,x2,y2;    //x1 and x2 are colatitude in degree, y1 is longitude;
{
double xxn1,xxn2,yyn1,yyn2,zzn1,zzn2,dist;
const double conv=M_PI/180.0;

// fprintf(stderr,"%g %g %g %g",x1,x2,y1,y2);

  x1 = x1*conv; // (90-x1)*conv; //convert to colati
  x2 = x2*conv; //(90-x2)*conv;
  y1 = y1*conv;
  y2 = y2*conv;
  ...
  zzn1 = cos(x1);
  zzn2 = cos(x2);
  dist = 0.5*sqrt( (xxn1-xxn2)*(xxn1-xxn2)
                  +(zzn1-zzn2)*(zzn1-zzn2)
                  +(yyn1-yyn2)*(yyn1-yyn2) );
  dist = 2.0*asin(dist);
