...
struct SPHERE   {
  ...
  int hindice;
  float *harm_geoid[2];
  float ***harm_field; //20170630 lhy spectrum
...
struct SLICE {    /* horizontally sliced data, including topography */
	int correct_mc; //20180720 whether correct topography with mass center
    float *tpg[NCS];
    float *tpgmc[NCS]; //20180720 after correcting mass center
    float *tpgb[NCS];
	float ***temp; //20160630 lhy spectrum
	float ***vr;
    float *shflux[NCS];
    float *bhflux[NCS];
    float *divg[NCS];
...
struct CONTROL {
    ...
    char sol_liq_dir[200];
	char sol_liq_file[150]; //lhy solidus_and_liqudus
	...
	int impact_added; //haoyuanli 20180104
  ...
  float  ggrd_lower_depth_km,ggrd_lower_scale,ggrd_lower_offset;
	...
	//lhy update_cmb_T
	int coreT;
	int inner_core_latent;
	char core_liquidus_file[250];
	//lmm melt_fraction
	int melting_model,melt_latent_heating,MF_save_step,fertility;
	float fco2,water,Mcpx; //lmm melt_fraction
	float F_inf;
	char background_profile_file[1000];
	float T_IBC_reset;
	float t_ic_sol;
	int need_init_sol;
	int output_area;
...
struct IMPACT{ //lhy 20180104 for impact modeling
	float Db; //diameter of impact crater [m]
	float Dc; //simple-to-complex diameter [m]
	float vi; //impact velocity [m/s]
	float C; //mantle P-wave speed [m/s]
	float S; //constant
};
...
struct Output {
	...
	int field;        /* 20170630 lhy spectrum; whether to output temperature/velocity spherial harmonics */
  ...
  int write_q_files;
  FILE *fpqt,*fpqb,*fpv;		/* additional heat flux output */
};
...
struct LUNAR{
	...
	int smooth_downwelling;         //zwb 20200730
	float smooth_cdepv[10];        //zwb 20200730
};
...
#include "advection.h"
	...
	struct CORE core; //lhy updata_cmb_T
    struct MONITOR monitor;
    struct DATA data;
	struct IMPACT impact;
    struct SLICE slice;
    ...
    double *T[NCS],*Tdot[NCS],*Tdiff[NCS],*buoyancy[NCS];
	double *melting[NCS];
	double *sol[NCS], *liq[NCS], *liq_l[NCS], *sol_l[NCS], *gsol[NCS], *melt_el[NCS], *rho_node[NCS]; //lhy solidus_liquidus
	double melting_volume, total_mass, T_avg;
	double *comp_melting_volume;
	double mc[4]; //lhy mass center coordinate(x,y,z)
	double *tide_force[NCS]; //lhy 20180115
    double *u1[NCS];
  ...
  //float *stress[NCS];
	...
	float *VIextra[NCS];	//lhy new_smoothing_method
  ...
  float *F0_el,*F_el; //mml melt fraction
  float *F0node,*Fnode;
  float *melt_latent_heat;
  float *UdotgradF;
};
