    ...
    struct All_variables *E;
{
  ...
  if (E->parallel.me_loc[3]==E->parallel.nprocz-1)   {
    sum_h[0] = sum_h[0]/sum_h[1];
  	for(m=1;m<=E->sphere.caps_per_proc;m++) //lhy boundary heat flux
		E->sphere.cap[m].heat_flux = sum_h[0];
...
/*     if (E->control.verbose && E->parallel.me==0) fprintf(E->fp_out,"bottom heat flux= %f %f\n",sum_h[2],E->monitor.elapsed_time); */
  	for(m=1;m<=E->sphere.caps_per_proc;m++) //lhy boundary heat_flux
		E->sphere.cap[m].heat_flux = sum_h[2];

    if (E->parallel.me==0)
      fprintf(stderr,"bottom heat flux= %f\n",sum_h[2]);
  }

/*get average temperature, lhy 20180713*/
  E->T_avg = return_bulk_value_d(E,E->T,1);
