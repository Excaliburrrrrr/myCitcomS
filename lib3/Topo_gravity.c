...
void get_vr_temp_slice(E,vr,temp) //20160630 lhy spectrum
    struct All_variables *E;
	float ***vr,***temp;
{
	int m,k,snode,nnode;
	...
	for(k=1;k<=E->lmesh.noz;k++)
   	for(m=1;m<=E->sphere.caps_per_proc;m++)
     	for(snode=1;snode<=E->lmesh.nsf;snode++)   {
			nnode=(snode-1)*E->lmesh.noz+k;
			temp[k][m][snode]=E->T[m][nnode];
			vr[k][m][snode]=E->sphere.cap[m].V[3][nnode];
		}
}
...
static void expand_vr_temp_sph_harm(struct All_variables *E,
                                 float ***field) //20170630 lhy spectrum
{
    int k;
    /* expand bottom topography into sph. harm. */
	for(k=1;k<=E->lmesh.noz;k++){
   		 sphere_expansion(E, E->slice.temp[k], field[k][0], field[k][1]);
   		 sphere_expansion(E, E->slice.vr[k], field[k][2], field[k][3]);
    	/* send arrays to all processors in the same vertical column */
    	broadcast_vertical(E, field[k][0], field[k][1], E->parallel.me_loc[3]);
    	broadcast_vertical(E, field[k][2], field[k][3], E->parallel.me_loc[3]);
	}
}
...
void compute_field(E) //20160630 lhy spectrum
     struct All_variables *E;
{
	get_vr_temp_slice(E,E->slice.vr,E->slice.temp);
    expand_vr_temp_sph_harm(E, E->sphere.harm_field);
	if(E->parallel.me==0)
		fprintf(stderr,"compute sph field for vr and T\n");
}
