...
void get_buoyancy(struct All_variables *E, double **buoy)
{
	...
	temp2 = E->composition.buoyancy_ratio[j] * temp;
            for(m=1;m<=E->sphere.caps_per_proc;m++)
	      for(i=1;i<=E->lmesh.nno;i++)
		buoy[m][i] -= temp2 *E->Have.GA_Moon[nz] * E->composition.comp_node[m][j][i];
      }
    }
#ifdef USE_GGRD
...
void tide_input(struct All_variables *E)
{
	int m = E->parallel.me;
	int n;
	input_int("planet_tide", &(E->tide.planet_tide), "0", m);
    input_int("num_tide", &n, "0", m);

    if (n > 0) {
      E->tide.n = n;
	  E->tide.ll = (int*)malloc(n*sizeof(int));
      if (! input_int_vector("tidel", n, E->tide.ll, m) ) {
		fprintf(stderr,"Missing input parameter: 'tidel'\n");
		parallel_process_termination();
      }
	}
...
void get_tide(struct All_variables *E, double **tide)
{
    int i,j,m,n,nz,nxny,ll,tiden;
    int lev = E->mesh.levmax;
    double rho, ratio, theta, fact, temp;
    void remove_horiz_ave2(struct All_variables*, double**);
	...
	nxny = E->lmesh.nox*E->lmesh.noy;
    temp = E->control.Atemp*E->data.ref_viscosity*E->data.therm_diff/(E->data.density
	*E->data.grav_acc*pow(E->data.radius,3));
	if(E->parallel.me==0){
		fprintf(stderr,"temp: %.4e\n",temp); //lhy debug
	}
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)
      for(i=1;i<=E->lmesh.nno;i++) {
		...
		theta = E->sx[m][1][i];
		fact = sqrt(2*M_PI*(ll*ll+ll)/(2*ll+1)/(ll-1));
		if(E->parallel.me==0&&i==1){
			fprintf(stderr,"factor in tidal force: %.4e\n",fact); //lhy debug
		}
