...
void output_surf_area(struct All_variables *);
void output_surf_area1(struct All_variables *);
...
void output_field(struct All_variables *, int); //20170630 lhy spectrum
...
void output(struct All_variables *E, int cycles)
{
  ...
  if (cycles == 0) {
    output_coord(E);
    output_domain(E);
	if(E->control.output_area == 1)
		output_surf_area(E);
	else if(E->control.output_area == 2)
		output_surf_area1(E);
  ...
  if (E->output.field)
	  output_field(E, cycles); //20170630 lhy spectrum
...
void output_surf_botm(struct All_variables *E, int cycles)
{
  ...
  void correct_for_mc();
  int i, j, s, ii, ll, mm ,pp, ll_max, chatty, lnsf;
  char output_file[255], output_sph[255];
  FILE *fp2, *fp_sph;
  float *topo, **topo1;
  double **topo_sph, *topo_sph_sum;

  chatty = E->parallel.me == 1;
  ll_max = E->output.llmax;
  lnsf = E->lmesh.nsf;
...
if(E->control.use_cbf_topo){
  ...
  if (E->output.surf && (E->parallel.me_loc[3]==E->parallel.nprocz-1)) {
    sprintf(output_file,"%s.surf_ori.%d.%d", E->control.data_file,
            E->parallel.me, cycles);
 ...
 /*output result for sph of dynamic topography*/ 
  if (E->output.surf && (E->parallel.me_loc[3]==E->parallel.nprocz-1)) {
	/*correct topography with mass center*/
  	if (E->slice.correct_mc){
  		correct_for_mc(E,E->slice.tpg,E->mc);
		topo1 = E->slice.tpgmc;
  	}
	else{
		topo1 = E->slice.tpg;
	}
	/*initialize for sphere expansion for topography*/
  	topo_sph=(double**)malloc((ll_max+1)*sizeof(double*));
  	for (ll=0;ll<=ll_max;ll++){
	 	topo_sph[ll] = (double*)malloc((lnsf+1)*sizeof(double));
  	}
  	topo_sph_sum=(double*)malloc((lnsf+1)*sizeof(double));
	/*expansion topography*/
	topo_sphere_expansion(topo_sph_sum,topo_sph,E,topo1);
	/*output sph coefficients*/
	if(E->parallel.me==1){
    	sprintf(output_sph,"%s.surf_sph.%d", E->control.data_file,cycles);
		fp_sph=output_open(output_sph,"w");
		for(ll = 0; ll<=ll_max ; ll++)
		for(mm = 0; mm<=ll; mm++){
			pp = E->sphere.hindex[ll][mm];
			fprintf(fp_sph,"%d %d %.4e %.4e\n", ll, mm, E->sphere.harm_tpgt[0][pp], E->sphere.harm_tpgt[1][pp]);			
		}
		fclose(fp_sph);
		//fprintf(stderr,"output surf_sph done\n"); //debug
	}
	/*output results*/
	...
	fp2 = output_open(output_file, "w");
	for(j=1;j<=E->sphere.caps_per_proc;j++)  {
        fprintf(fp2,"%.4e %3d %7d\n",topo_sph[0][1],j,E->lmesh.nsf);
        for(i=1;i<=E->lmesh.nsf;i++)   {
            s = i*E->lmesh.noz;
            fprintf(fp2,"%.4e %.4e %.4e %.4e %4e %.4e\n",
		     topo1[1][i],topo_sph[1][i],topo_sph[2][i],topo_sph[3][i],topo_sph[4][i],topo_sph_sum[i]); 
        }
    }
    fclose(fp2);
	/*if(E->parallel.me==1){
		fprintf(stderr,"output surf done\n"); //debug
	}*/
	/*free space*/
	for(ll=0;ll<=ll_max;ll++)
		free(topo_sph[ll]);
	...
	/*output for corected topography*/
  return;
}
...
void output_field(struct All_variables *E, int cycles) 
//20170630 lhy spectrum expansion
{
    void compute_field();
    int ll, mm, p, k, noz;
    char output_file[255];
    ...
    if (E->parallel.me <= (E->parallel.nprocz-1))  {
        sprintf(output_file, "%s.field.%d.%d", E->control.data_file,
                E->parallel.me, cycles);
        fp1 = output_open(output_file, "w");
		...
		for (k=1; k<=E->lmesh.noz; k++)	{
			noz=E->parallel.me_loc[3]*(E->lmesh.noz-1)+k;
			fprintf(fp1,"%d %.4e\n",noz,E->sphere.gr[noz]);
	        for (ll=0; ll<=E->output.llmax; ll++)
	            for(mm=0; mm<=ll; mm++)  {
	                p = E->sphere.hindex[ll][mm];
	                fprintf(fp1,"%d %d %.4e %.4e %.4e %.4e\n",
	                        ll, mm,
	                        E->sphere.harm_field[k][0][p],
	                        E->sphere.harm_field[k][1][p],
	                        E->sphere.harm_field[k][2][p],
	                        E->sphere.harm_field[k][3][p]);
	            }
		}
...
void output_horiz_avg(struct All_variables *E, int cycles)
{
  ...
  if (E->parallel.me<E->parallel.nprocz)  {
    ...
    for(j=1;j<=E->lmesh.noz;j++)  {
        fprintf(fp1,"%.4e %.4e %.4e %.4e %.4e",E->sx[1][3][j],
		E->Have.T[j],E->Have.V[1][j],E->Have.V[2][j],E->Have.vis[j]);
...
void boundary_write_q_files(struct All_variables *E)
{
	  int i;
	  double temp;
	  /*
	  if(E->parallel.me==0){
		  fprintf(stderr,"begin boundary_write_q_files\n");
	  }
	  */
      if(E->output.write_q_files > 0){
		/* format: time heat_flow sqrt(v.v)  */
  		if (E->parallel.me==E->parallel.nprocz-1)   {
			fprintf(E->output.fpqt,"%13.5e %13.5e %13.5e\n",E->monitor.elapsed_time,E->sphere.cap[1].heat_flux,sqrt(E->monitor.vdotv));
			...
			fprintf(E->output.fpqb,"%13.5e %13.5e %13.5e",
				E->monitor.elapsed_time,E->sphere.cap[1].heat_flux,sqrt(E->monitor.vdotv));
	  		if (E->control.coreT)
				fprintf(E->output.fpqb," %13.5e %13.5e",E->core.Tcmb,E->core.rin); 
			fprintf(E->output.fpqb,"\n");
			fflush(E->output.fpqb);
			/*---------------volume result goes here--------*/
			fprintf(stderr,"begin output volume file\n"); 
			fprintf(E->output.fpv,"%d ",E->monitor.solution_cycles);//debug
			//fprintf(E->output.fpv,"%13.5e",E->monitor.elapsed_time);
			fprintf(E->output.fpv," %13.5e",E->T_avg);
			fprintf(E->output.fpv," %13.5e %13.5e %13.5e",E->mc[1],E->mc[2],E->mc[3]);
			if (E->convection.sol_liq){
				fprintf(E->output.fpv," %13.5e",E->melting_volume);
				temp = E->melting_volume;
				for(i=0;i<E->composition.ncomp;i++){
					temp -= E->comp_melting_volume[i];
				}
				fprintf(E->output.fpv," %13.5e",temp);
				for(i=0;i<E->composition.ncomp;i++){
					fprintf(E->output.fpv," %13.5e",E->comp_melting_volume[i]);
				}
			}
			fprintf(E->output.fpv,"\n");
			fflush(E->output.fpv);
		}
	  }	
}
