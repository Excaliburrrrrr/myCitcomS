...
double visc_from_chemical(struct All_variables *, double *, double *);
...
{
  if(E->parallel.me == 0)
	  fprintf(stderr,"melting_initialize\n");
  int m,i;
  for(m=1;m<=E->sphere.caps_per_proc;m++) {
...
void read_sol_liq_from_file(struct All_variables *E)
{
  ...
  for(m=1;m<=E->sphere.caps_per_proc;m++) {
	if (E->parallel.me == 0)
		fprintf(stderr,"read_sol_liq_from_file\n"); //lhy debug
    ...
    if(E->parallel.me==1){ //lhy debug
	fprintf(stderr,"solidus liquidus for #1\n");
    	for(i=1;i<=noz;i++){
	    fprintf(stderr,"%d sol = %.4e, liq = %.4e\n",i,E->sol_l[m][i],E->liq_l[m][i]);
	}
    }
  }
	fclose(fp);
...
static void all_minerals(struct All_variables *E){
	...
	int num;
	E->mineral.num = 5; /*1 for harzburgite mantle , 2 for ilmenite, 3 for clinopyroxene,4 for anorthsite crust*/
	num = E->mineral.num;
/*initialize*/
...
static void all_chemicals(struct All_variables *E){
	...
	if(E->lunar.model_type==1)
		...
		E->trace.ic_flavor = 1;
	num = E->trace.nflavors;
	E->chemical.frac = (double**)malloc(num*sizeof(double*));
	E->chemical.interface = (double**)malloc(num*sizeof(double*));
	E->chemical.buoyancy = (double*)malloc(num*sizeof(double));
	for(i=0;i<num;i++){
...
/* read input data */
    ...
    //fprintf(E->fp_out,"Initial Radial Temperature Field: %s\n",E->akm.background_profile_file);
...
/* now fill citcom nodes */
    ...
    //fprintf(E->fp_out,"Initial temperature profile\n");
    for (kk=1;kk<=noz;kk++)
    {
       //fprintf(E->fp_out,"Radius: %f Temperature: %f\n",E->sx[1][3][kk],E->T[1][kk]);
       //fflush(E->fp_out);       
    }
    if (E->parallel.me==0)
    {
      //fprintf(E->fp_out,"Initial temperature profile (proc 0)\n");
      for (kk=1;kk<=noz;kk++)  {
        fprintf(stderr,"Radius: %f Temperature: %f\n",E->sx[1][3][kk],E->T[1][kk]);
        fflush(stderr);
      }
    }
