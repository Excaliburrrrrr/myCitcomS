     ...
     int penalty;
{
  ...
  if(E->tide.planet_tide) //lhy 20180115
  get_tide(E,E->tide_force);
...
/* VBX is 0x2, VBY is 0x8 VBZ is 0x4
*/

  double force[9],force_at_gs[9],elt_k[24*24];
  double force_t[9], force_at_gs_t[9]; //lhy 20180115


  const int dims=E->mesh.nsd; //3 for 3d
  const int n=loc_mat_size[dims]; //24 for dim=3
  const int ends=enodes[dims]; //8 for dim=3
  const int vpts=vpoints[dims]; //8 for dim =3
  ...
  for(p=1;p<=ends;p++){
	...
	if(E->tide.planet_tide) //lhy 20180115
	force_t[p] = E->tide_force[m][E->ien[m][el].node[p]];
  }

  for(j=1;j<=vpts;j++)       {   /*compute force at each int point */
    force_at_gs[j] = 0.0;
	force_at_gs_t[j] = 0.0; //lhy 20180115
    for(k=1;k<=ends;k++)
      force_at_gs[j] += force[k] * E->N.vpt[GNVINDEX(k,j)] ;
	  if(E->tide.planet_tide)
      force_at_gs_t[j] += force_t[k] * E->N.vpt[GNVINDEX(k,j)] ;
    }
    ...
    for(a=1;a<=ends;a++)  {
      ...
      for(j=1;j<=vpts;j++){     /*compute sum(Na(j)*F(j)*det(j)) */
        elt_f[p] += force_at_gs[j] * E->N.vpt[GNVINDEX(a,j)]
	 	...
	 	if(E->tide.planet_tide)
        elt_f[p] += force_at_gs_t[j] * E->N.vpt[GNVINDEX(a,j)]
           *E->gDA[m][el].vpt[j]*g_point[j].weight[dims-1]
           *E->element_Cc.vpt[BVINDEX(1,i,a,j)];
	  }
