...

{
	...
	E->sphere.harm_field=(float***)malloc((E->lmesh.noz+1)*sizeof(float**));//20170630 lhy spectrum
	for (k=1;k<=E->lmesh.noz;k++){
		E->sphere.harm_field[k]=(float**)malloc(4*sizeof(float*));
		for (i=0;i<=3;i++)
			E->sphere.harm_field[k][i]=(float*)malloc(E->sphere.hindice*sizeof(float));
	}
