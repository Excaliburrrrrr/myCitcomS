  ...
  int lev;
{
  ...
  for(m=1;m<=E->sphere.caps_per_proc;m++)
    for(n=1;n<=E->lmesh.NNO[lev];n++)
      VN[m][n] *= E->MASS[lev][m][n]; //lhy something wrong here
...
void visc_from_nodes_to_gint_log(E,VN,VE,lev)
  struct All_variables *E;
  float **VE,**VN;
  int lev;
{
  ...
  for (m=1;m<=E->sphere.caps_per_proc;m++)
	...
	temp_visc=0.0;
	for(j=1;j<=ends;j++)
	  temp_visc += E->N.vpt[GNVINDEX(j,i)]*log(VN[m][E->IEN[lev][m][e].node[j]]);
	VE[m][(e-1)*vpts+i] = exp(temp_visc);
      }
