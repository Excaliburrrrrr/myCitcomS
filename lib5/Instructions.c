...
static void impact_input(struct All_variables*);
...
void read_instructions(struct All_variables *E, char *filename)
{
	...
	/*if(E->parallel.me==0){
		fprintf(stderr,"begin setup_parser\n");
	}*/ //debug
    setup_parser(E,filename);
	/*if(E->parallel.me==0){
		fprintf(stderr,"begin read_initial_settings\n");
	}*/ //debug
...
{
  void print_all_config_parameters(); //debug
  void set_convection_defaults();
  void set_cg_defaults();
  void set_mg_defaults();
  void tide_input();//lhy 20180116
  void isotope_input();//lhy 20180305
  void initial_update_cmb_T(); //lhy update_cmb_T
  ...
  */
  ...
  input_boolean("need_init_sol",&(E->control.need_init_sol),"on",m);
  input_int("output_area",&(E->control.output_area),"0",m);
  ...
  */
  ...
  input_int("write_q_files",&(E->output.write_q_files),"0",m);/* write additional
								 heat flux files? */
  input_int("correct_mc",&(E->slice.correct_mc),"0",m);
  ...
  input_float("surftemperature",&(E->data.Tsurf),"273.15",m);
  input_float("reftemperature",&(E->data.ref_temperature),"3000.0",m);
  ...
  */
...
#endif
  input_float("radius",&(E->data.radius),"6371e3.0",m); //lhy 20180104
  //mml melt_fraction
  input_int("melting_model",&(E->control.melting_model),"0",m);
  input_int("melt_latent_heating",&(E->control.melt_latent_heating),"0",m);
  input_int("MF_save_step",&(E->control.MF_save_step),"100",m);
  input_float("melt_inf",&(E->control.F_inf),"0.02",m);
  input_float("water",&(E->control.water),"0.01",m);
  input_float("Mcpx",&(E->control.Mcpx),"0.15",m);
  input_int("fertility",&(E->control.fertility),"1",m);
  input_float("fco2",&(E->control.fco2),"0.0",m);
  ...
  E->data.therm_exp = E->control.Atemp * E->data.therm_diff
    * E->data.ref_viscosity
    / (E->data.density * E->data.grav_acc * E->data.ref_temperature)
  ...
  if(E->parallel.me==0){
	  fprintf(stderr,"begin specified_model_initialize\n");
  } //debug
  //fprintf(stderr,"1\n"); //debug
  specified_model_initialize(E); //added by lhy for model specified parameters
  return;
}

static void impact_input(struct All_variables *E){
	int m=E->parallel.me;
	input_boolean("impactadded",&(E->control.impact_added),"off",m);
	input_float("impactDb",&(E->impact.Db),"2000e3",m);
	input_float("impactDc",&(E->impact.Dc),"20e3",m);
	input_float("impactvi",&(E->impact.vi),"10e3",m);
	input_float("impactC",&(E->impact.C),"7.7e3",m);
	input_float("impactS",&(E->impact.S),"1.25",m);
}
...
void global_derived_values(struct All_variables *E)
{
  ...
  /* this is a rough estimate for global neq, a more accurate neq will
   ...
   /* Scaling from dimensionless units to Millions of years for input velocity
	...
	sprintf(mffile, "%s.mf.dat", E->control.data_file);
...

{
 ...
 for (j=1;j<=E->sphere.caps_per_proc;j++)  {
  ...
  E->melt_latent_heat = (float *)malloc((nel+1)*sizeof(float));
  E->Tdiff[j]        = (double *) malloc((nno+1)*sizeof(double));
  for (i=1;i<=nno;i++){//assign initial zero value to E->Tdiff
	  E->Tdiff[j][i] = 0.0;
  }
  E->NP[j]       = (float *) malloc((nno+1)*sizeof(float));
  E->buoyancy[j] = (double *) malloc((nno+1)*sizeof(double));
  E->tide_force[j] = (double *) malloc((nno+1)*sizeof(double));
  ...
  E->slice.tpgmc[j]    = (float *)malloc((nsf+2)*sizeof(float));
  ...
  for(k=1;k<=E->lmesh.noz;k++){
	  ...
	  for(j=1;j<=E->sphere.caps_per_proc;j++){
	  	E->slice.temp[k][j]      = (float *)malloc((nsf+2)*sizeof(float));
	  	E->slice.vr[k][j]      = (float *)malloc((nsf+2)*sizeof(float));
	  }
  }
					     ...
					     flux output */
{
  ...
  if (E->parallel.me == 0){
	  sprintf(output_file,"%s.volume.dat", E->control.data_file);
    if(E->control.restart)
      E->output.fpv = output_open(output_file, "a"); /* append */
    else
      E->output.fpv = output_open(output_file, "w");
  }
...
static void output_parse_optional(struct  All_variables *E)
{
    ...
    while(1) {
        ...
        else if(strcmp(prev, "field")==0) //20170630 lhy spectrum
	    if (E->parallel.nprocxy != 12) {
		fprintf(stderr, "Warning: field specturm calculation only works in full version. Disabled\n");
	    }
	    else {
		E->output.field  = 1;
	    }
            ...
            E->output.heating = 1;
        else if(strcmp(prev, "tide")==0) //lhy 20180116
            E->output.tide = 1;
        else
            if(E->parallel.me == 0)
...
void print_all_config_parameters(struct All_variables *E)
{
  ...
  if(E->parallel.me == 0) {
    ...
    {
      for(i=0; i<E->convection.heat_sources.number-1;i++)
        fprintf(fp, "%g,", E->convection.heat_sources.Q[i]);
    ...
    fprintf(fp, "isotope_QER=");
    if(E->convection.heat_sources.number > 0)
    {
      for(i=0; i<E->convection.heat_sources.number-1;i++)
        fprintf(fp, "%g,", E->convection.heat_sources.QER[i]);
    ...
    fprintf(fp, "isotope_lambda=");
    if(E->convection.heat_sources.number > 0)
    {
      for(i=0; i<E->convection.heat_sources.number-1;i++)
        fprintf(fp, "%g,", E->convection.heat_sources.lambda[i]);
